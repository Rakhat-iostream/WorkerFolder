...
namespace QueueAPI.Repositories
{
    public class QueueRepository : IQueueRepository
    {
        ...
        {
            return await context.Messages.FirstOrDefaultAsync(x => x.Handled == false && x.Type.Equals(type));
        }


        public async Task<bool> SetHandled(Guid messageId)
        {
            var message = context.Messages.Where(x => x.Id == messageId).FirstOrDefault();
            if (message == null)
                return false;
